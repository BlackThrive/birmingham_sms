...
This document presents the findings for the first version of Black Thrive Global's Shared Measurement System dashboard for Birmingham. The aim of the dashboard is to quantify racial inequality in Birmingham by comparing various indicators for the Black population with the White population. Doing so makes it possible to identify the aspects of life in Birmingham where inequality is manifest and to highlight where change is needed. By routinely reassessing these indicators, it will also be possible to track if and how these inequalities change over time.

The indicators included in this dashboard were selected because they are the ones currently presented Black Thrive's [Lambeth dashboard](https://lambeth.blackthrive.org/shared-measurement-system/). The indicators are:
...
rr_gld <- rr_from_df(gld_df, "Good level of development at age 5 (2018-2019)") # get risk ratio
xtab_gld <- CrossTable(bw_mat, fisher = T, chisq = T, expected = T) # save xtab
...
# run stats
rr_gcse <- rr_from_df(gcse_df, "Level 4+ GCSE Maths & English (2018-2019)") # get risk ratio
xtab_gcse <- CrossTable(bw_mat, fisher = T, chisq = T, expected = T)
```
...
# run analysis
rr_cla <- rr_from_df(cla_df, "Children looked after by local authority (2018-2019)") # risk ratio
xtab_cla <- CrossTable(bw_mat, fisher = T, chisq = T, expected = T)
...
rr_employment <- rr_from_df(employment_df, "Employment rate (2020-2021)") # risk ratio

# run analysis
xtab_employment <- CrossTable(bw_mat, fisher = T, chisq = T, expected = T)
...
rr_stop_search <- rr_from_df(stop_search_df, "Police Stop and Search (2020-2021)") # risk ratio
...
rr_homelessness <- rr_from_df(homeless_df, "Statutorily homeless or at risk (2020-2021)") # risk ratio

# run analysis
xtab_homelessness <- CrossTable(bw_mat, fisher = T, chisq = T, expected = T)
```
...
black_df <- data.frame("indicator" = c("Employment", 
...
black_df$indicator <- factor(black_df$indicator, levels = c("Employment", 
                                                "Good level of development at age 5", 
                                                "Level 4+ GCSE Maths & English", 
                                                "Children looked after",
                                                "Police Stop and Search",
...
black_df_2$original_OR <- black_df_2$OR # keep original OR values

black_df_2 <- black_df_2[order(black_df_2$OR),] # order df from lowest to highest
black_df_2$indicator <- fct_inorder(black_df_2$indicator) # put levels in order they appear in df

reduction_factor <- 3 # set reduction factor
for(i in 1:nrow(black_df_2)){
  if(black_df_2$OR[i] < 1){ # expand values below 1
    black_df_2$OR[i] <- reduction_factor * (0 - (1 - black_df_2$OR[i]))
    black_df_2$ci_low[i] <- reduction_factor * (0 - (1 - black_df_2$ci_low[i]))
    black_df_2$ci_upp[i] <- reduction_factor * (0 - (1 - black_df_2$ci_upp[i]))
  }
  else{ # transform values above one
      black_df_2$OR[i] <- black_df_2$OR[i] - 1 
    black_df_2$ci_low[i] <- black_df_2$ci_low[i] - 1
    black_df_2$ci_upp[i] <- black_df_2$ci_upp[i] - 1
  }
}

ggplot(black_df_2, aes(indicator, OR)) +
  geom_hline(yintercept = 0, colour = bt_orange, size = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_upp), width = 0.2) +
  scale_y_continuous(expand = c(0, 0), limits = c(reduction_factor * -1, 3), breaks = c((reduction_factor * -1) * 0.9, 0.5 * reduction_factor - reduction_factor, 0.75 * reduction_factor - reduction_factor, 0, 0.5, 1, 2), labels = c(
    "10x less likely",
    "Half as likely",
    "3/4 as likely",
    "Just as likely",
  ...
  annotate("segment", x = nrow(black_df) + 1, xend = nrow(black_df) + 1, y = 0.1, yend = reduction_factor * 0.9, arrow = arrow(angle = 45), size = 1) +
  geom_text(x = nrow(black_df) + 1.5, y = reduction_factor*.9 - .3, label = "More likely", colour = "black", size = 3.5, hjust = 1) +
  annotate("segment", x = 7, xend = 7, y = -0.1, yend = -1*reduction_factor*0.9, arrow = arrow(angle = 45), size = 1) +
  geom_text(x = nrow(black_df) + 1.5, y = -1*reduction_factor*0.9 +.3, label = "Less likely", colour = "black", size = 3.5, hjust = 0) +
...
black_df_3 <- black_df_3[order(-c(black_df_3$OR)),]

black_df_3 %>% kable("html", escape = F, row.names = F, col.names = linebreak(c("Indicator", "Odds Ratio", "Lower confidence<br>interval (95%)", "Upper confidence<br>interval (95%)")), caption = "Odds ratios and confidence intervals corresponding to the above plot") %>% kable_styling(full_width = F)
```
...
* Black pupils were `r round(100 * (1 - rr_df$rr[which(rr_df$short_name == "gcse")]), 2)`% less likely than White pupils to achieve at least Level 4 in GCSE Maths and English.
* Black children were no more or less likely than White children to reach a Good Level of Development at age 5.
* Black children were no more or less likely that White children to be looked after by the local authority.
...
### Pupils achieving Level 4 and above in GCSE Maths and English
...
In the academic year 2019-2020, `r round(plot_data["black","percent_4_plus"],2)`% of Black pupils and `r round(plot_data["white","percent_4_plus"],2)`% of White pupils achieved at least Level 4 in GCSE Maths and English. Black pupils were `r round(100 * (1 - rr_df$rr[which(rr_df$short_name == "gcse")]), 2)`% less likely than White pupils to achieve at least Level 4 in GCSE Maths and English.
...
In the 2018/2019 academic year, `r round(plot_data["Black","gld_percent"],2)`% of Black children, and  `r round(plot_data["White","gld_percent"],2)`% of White children, were assessed as reaching a Good Level of Development at age 5. The difference between Black and White children is not statistically significant.
...
In the 2018-2019 financial year, `r round(total_data["Black","percentage"],2)`% of Black children, and `r round(total_data["White","percentage"],2)`% of White children, were looked after by the local authority. This difference is not statistically significant.
...
ggplot(plot_data, aes(ethnicity, cla)) + 
  geom_col(colour = "black", fill = "#ff9000", width = .5) +
  scale_y_continuous(label = comma, expand = c(0,0), limits = c(0, maxval * 1.15)) + 
  geom_line(data = line_df, aes(a, b)) +
  annotate("text", x = mpos, y = txtval, label = paste0("Difference = ", difference, "%")) +
...
The R code that produced this report is available on Github [here](https://github.com/BlackThrive/birmingham_sms). A separate methodology paper that provides more detail on the data and analysis procedure is available [here](https://github.com/BlackThrive/sms_methodology).
